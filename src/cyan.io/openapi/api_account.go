...
/*
AccountApiService Retrieves account balances information.
 ...
 * @param timestamp Mandatory field for retrieving account balances in query string ?recvWindow=5000×tamp=1540203005798. The future release of API generate use timestamp provided by user in conjunction with recvWindow parameter provided by the user to check if request has reached in time. If the server timestamp is later than sun of value of timestamp and recvWindow by the user  then request will be rejected.
 * @param optional nil or *AccountBalancesHMACSHA256GetOpts - Optional Parameters:
 * @param "RecvWindow" (optional.Int32) -  Optional field for retrieving account balances in query string ?recvWindow=5000×tamp=1540203005798.
@return []AccountBalance
*/

type AccountBalancesHMACSHA256GetOpts struct {
	RecvWindow optional.Int32
}

func (a *AccountApiService) AccountBalancesHMACSHA256Get(ctx context.Context, timestamp int64, localVarOptionals *AccountBalancesHMACSHA256GetOpts) ([]AccountBalance, *http.Response, error) {
	var (
	...
	localVarPath := a.client.cfg.BasePath + "/account/balances"
	...
	if localVarOptionals != nil && localVarOptionals.RecvWindow.IsSet() {
		localVarQueryParams.Add("recvWindow", parameterToString(localVarOptionals.RecvWindow.Value(), ""))
	}
	...
	println(localVarHttpContentType)
	...
	println("Response is: ", localVarHttpResponse.Header.Get("Content-Type"))
...
/*
AccountApiService Retrieves account details information.
 ...
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param timestamp Mandatory field for retrieving account details in query string ?recvWindow=5000×tamp=1540203005798.If the server timestamp is later than sum of value of timestamp and recvWindow provided by the user  then request will be rejected.
 * @param optional nil or *AccountDetailsHMACSHA256GetOpts - Optional Parameters:
 * @param "RecvWindow" (optional.Int32) -  Optional field for retrieving account balances in query string ?timestamp=1540203005798&recvWindow=5000.
@return AccountDetails
*/

type AccountDetailsHMACSHA256GetOpts struct {
	RecvWindow optional.Int32
}

func (a *AccountApiService) AccountDetailsHMACSHA256Get(ctx context.Context, timestamp int64, localVarOptionals *AccountDetailsHMACSHA256GetOpts) (AccountDetails, *http.Response, error) {
	var (
		localVarHttpMethod   = strings.ToUpper("Get")
		localVarPostBody     interface{}
		localVarFormFileName string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/account/details"
